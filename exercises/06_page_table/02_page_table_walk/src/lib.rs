...
impl SingleLevelPageTable {
    ...
    pub fn translate(&self, va: u32, is_write: bool) -> TranslateResult {
        ...
        match self.lookup(vpn) {
            Some(ppte) => {
                if ppte.flags & PTE_VALID == 0 {
                    TranslateResult::PageFault
                } else {
                    if is_write && ppte.flags & PTE_WRITE == 0{
                        TranslateResult::PermissionDenied
                    } else {
                        TranslateResult::Ok(ppte.ppn * PAGE_SIZE as u32 + offset)
                    }
                }
            },
            None => {
