...
unsafe impl GlobalAlloc for FreeListAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        loop {
            ...
            let align_start = self.align_up(next, align);
            // 3. Compute allocation end = aligned + layout.size()
            let alloc_end = align_start + size;
            ...
            if alloc_end > self.heap_end { 
                ...
                match self.bump_next.compare_exchange(next, alloc_end, Ordering::SeqCst, Ordering::SeqCst) {
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        let dealloced_blk = ptr as *mut FreeBlock;
        // 2. Write FreeBlock { size, next: current list head }
        (*dealloced_blk).size = size;
        (*dealloced_blk).next = self.free_list_head();
        // 3. Update free_list head to ptr
        self.set_free_list_head(dealloced_blk);
    }
}
