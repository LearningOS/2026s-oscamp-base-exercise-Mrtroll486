...
impl AtomicCounter {
    ...
    pub fn fetch_multiply(&self, multiplier: u64) -> u64 {
        ...
        // }
        loop {
            let old_val = self.value.load(Ordering::Acquire);
            let new_val = old_val * multiplier;

            match self.value.compare_exchange(old_val, new_val, Ordering::AcqRel, Ordering::Acquire) {
                Ok(_) => {
                    return old_val;
                },
                Err(_) => {
